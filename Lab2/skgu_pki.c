...
dckey* g_option(const char *priv_file)
{
    char *raw_pub = NULL;
    dckey *pub = NULL;
...
char* o_option(const char *c_file)
{
    return xstrdup(c_file);
}

int e_option(const char *days)
{
...
void parse_options(dckey **ppub, char **pcfile, int *pdur, int argc, char **argv)
{
    char opt;
    char *opt_arg;
    int arg_idx = 2;
